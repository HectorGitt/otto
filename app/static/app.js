class RealtimeDemo {
	...
	addSystemMessage(message, type = "info") {
		...
		bubbleDiv.className = "message-bubble system-message";
	...
	initializeElements() {
		...
		this.connectBtnIcon = this.connectBtn.querySelector("i");
		this.muteBtn = document.getElementById("muteBtn");
		this.muteBtnIcon = this.muteBtn.querySelector("i");
		this.muteBtnText = this.muteBtn.querySelector("span");
	...
	setupEventListeners() {
		...
		document.addEventListener("keydown", (e) => {
			// Ctrl/Cmd + Enter to send image if prompt has content
			if ((e.ctrlKey || e.metaKey) && e.key === "Enter") {
				if (this.imagePrompt.value.trim()) {
					this.imageInput.click();
				}
			}
			// Space to toggle mute when connected
			if (
				e.code === "Space" &&
				this.isConnected &&
		...
		this._handlePickedFile = async (file) => {
			try {
				...
				const promptText =
					(this.imagePrompt && this.imagePrompt.value) || "";
				...
				if (this.ws && this.ws.readyState === WebSocket.OPEN) {
					...
					for (let i = 0; i < dataUrl.length; i += CHUNK) {
						...
						this.ws.send(
							JSON.stringify({ type: "image_chunk", id, chunk })
						);
					}
	...
	async connect() {
		...
		try {
			...
			this.connectBtnIcon.className = "fas fa-spinner loading";
			...
			this.ws.onopen = () => {
				...
				this.addSystemMessage(
					'🎉 Connected! You can now speak to Otto. Try saying "Open Notepad" or "Take a screenshot".',
					"success"
				);
			};
			...
			this.ws.onerror = (error) => {
				...
				this.updateConnectionUI("disconnected", "Connection Failed");
				this.addSystemMessage(
	...
	disconnect() {
		...
		this.isConnected = false;
		this.updateConnectionUI("disconnected", "Disconnected");
		this.addSystemMessage("👋 Disconnected from Otto.", "info");
	}
	...
	updateMuteUI() {
		if (this.isMuted) {
			...
			this.muteBtnIcon.className = "fas fa-microphone-slash";
			this.muteBtnText.textContent = "Mic Off";
		} else {
			this.muteBtn.className = "mute-btn unmuted";
			this.muteBtnIcon.className = "fas fa-microphone";
			this.muteBtnText.textContent = "Mic On";
			if (this.isCapturing) {
	...
	addToolEvent(event) {
		...
		if (event.type === "handoff") {
			...
			// Check if the tool execution was successful based on output
			const output = event.output || "";
			const isSuccess =
				!output.toLowerCase().includes("failed") &&
				!output.toLowerCase().includes("error");
			title = isSuccess ? `✅ Tool Completed` : `❌ Tool Failed`;
			description = `${event.tool}: ${output || "No output"}`;
			eventClass = `tool ${isSuccess ? "success" : "error"}`;
		}
	...
	async playAudioChunk(audioBase64) {
		return new Promise((resolve, reject) => {
			try {
				...
				const fade = Math.min(
					this.playbackFadeSec,
					Math.max(0.005, audioBuffer.duration / 8)
				);
				try {
					...
					gainNode.gain.setValueAtTime(
						1.0,
						Math.max(now + fade, endTime - fade)
					);
			...
			try {
				...
				if (this.currentAudioGain) {
					try {
						...
						this.currentAudioGain.gain.linearRampToValueAtTime(
							0.0001,
							now + fade
						);
					} catch {}
